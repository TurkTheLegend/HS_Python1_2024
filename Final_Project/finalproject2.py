...
def store_master_password(data, username, password):
    ...
    data[username] = {
        "master_password": (hashed_password.decode("utf-8"), salt.decode("utf-8")),
        "apps": {},
    }
    save_data(data)
...
def retrieve_app_password(data, username, app_name, account_name, master_password):
    ...
    if (
        ...
        if bcrypt.checkpw(
            ...
            encrypted_app_password, app_salt = data[username]["apps"][app_name][
                account_name
            ]
            key = bcrypt.kdf(
                password=master_password.encode("utf-8"),
    ...
    while True:
        ...
        if choice == "1":
            ...
            if username in data and bcrypt.checkpw(
                master_password.encode("utf-8"),
                data[username]["master_password"][0].encode("utf-8"),
            ):
                ...
                while True:
                    ...
                    if choice == "1":
                        ...
                        while password == "":
                            password = input(("Enter password: ")).strip()
                        store_app_password(
                            data, username, app_name, account_name, password, master_password
                        )
                        ...
                        try:
                            ...
                            chosen_account = list(
                                user_data["apps"][chosen_app].keys()
                            )[account_choice]
                        except (ValueError, IndexError):
                            print("Invalid account choice.")
                            continue

                        app_password = retrieve_app_password(
